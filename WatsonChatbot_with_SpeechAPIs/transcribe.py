...
auth = IAMAuthenticator('VVWB2jtjcwxFFFh4jLFZKSh0y11ylTqpZyxCe9AqiXBZ')
tts = TextToSpeechV1(authenticator=auth)

tts.set_service_url('https://api.us-south.text-to-speech.watson.cloud.ibm.com')
opening = 'Welcome, Iam Ms Evaluator ... How can I Help?'          
CHUNK = 1024
...
authent = IAMAuthenticator('tbN_9RzlABzUd3jmmC6LpQUHUKwreiT5PcrQkZUiNUUY')
assistant = AssistantV2(
            version='2020-04-01',
            authenticator = authent
        )
assistant_id= "6f757514-991f-4acb-a803-1707ffbe165d"
assistant.set_service_url('https://api.eu-de.assistant.watson.cloud.ibm.com/instances/1ceef69e-a67b-4c21-8e00-ced8aec59350')
...
def on_message(self, msg):
    ...
    if "results" in data:
        if data["results"][0]["final"]:
            ...
            with open('TTS.mp3','wb') as audio_file:
                audio_file.write(
                        tts.synthesize(response['output']['generic'][0]['text'],
                        voice='en-US_AllisonV3Voice',
                        accept='audio/mp3'        
                        ).get_result().content)
                
            answer = AudioSegment.from_mp3('TTS.mp3')
    ...
    with open("STT.txt", "w") as Text_file: Text_file.write(data['results'][0]['alternatives'][0]['transcript']) 
...
def get_url():
    ...
    return ("wss://{}/speech-to-text/api/v1/recognize"
           "?model=en-AU_NarrowbandModel").format(host)
...
def main(): 
    with open('opening.mp3','wb') as audio_file:
                audio_file.write(
                        tts.synthesize(opening,
                        voice='en-US_AllisonV3Voice',
                        accept='audio/mp3'        
                        ).get_result().content)
                
    answer = AudioSegment.from_mp3('opening.mp3')
    print(opening)
    play(answer)
